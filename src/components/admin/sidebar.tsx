...
const menuItems = [
  ...
  { name: 'Заказы', icon: List, href: '/admin/orders' },
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex items-center gap-3 px-5 py-3 text-[13px] transition-colors border-b border-gray-100 last:border-0",
                isActive ? "bg-[#EEEDFE] text-[#3C3489] font-medium" : "text-gray-500 hover:bg-gray-50"
              )}
            >
